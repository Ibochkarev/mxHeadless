...
mxHeadless не вызывает фронт синхронно в HTTP-запросе API. Доставка асинхронная через webhook worker.
...
Маршрут `app/api/revalidate/route.ts`:
...
1. Строка в `mxheadless_webhook_subscriptions`.
2. URL: `https://frontend.example/api/revalidate`
3. Events: `resources.*` или `*`
4. Secret: общий с фронтом
5. [Webhook worker](workers.md) в cron каждую минуту
