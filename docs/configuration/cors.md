...
| Key | Default | Notes |
|-----|---------|-------|
| `mxheadless.cors.enabled` | `false` | Master switch |
| `mxheadless.cors.allowed_origins` | empty | Comma-separated exact origins, or `*` |
...
| `mxheadless.cors.expose_headers` | ETag,X-Request-ID,X-RateLimit-*,Idempotency-Replayed | Browser JS can read these |
| `mxheadless.cors.allow_credentials` | `false` | Do not combine with `*` origins |
